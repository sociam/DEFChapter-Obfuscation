...
be framed as methods for empowering criminals, miscreants and even terrorists 
to more easily conduct their misdeeds without consequence.  However, as increasingly 
sophisticated methods for tracking, monitoring and predicting individuals have 
become pervasively deployed in order to extract economic value from their
personal information, the old ``I have nothing to hide; therefore I should not care'' 
argument has fallen apart. Previously theoretical arguments of what \emph{might} 
happen if one is pervasively tracked have been replaced by the tangible ability to 
witness hundreds of information brokers (some visible, others invisible), competing 
to build the most complete picture of each and every individual's life, so that such models 
can be used to precisely target, and behaviourally manipulate them, often in ways too 
subtle for them to perceive (e.g. \cite{facebookstudy}). 

The sophisticated methods being used to track and pinpoint details of individuals 
has not been met with a corresponding increase in awareness of how they are 
being tracked, nor a proliferation of counter-tracking and counter-surveillance tools. 
As a result, a great asymmetric divide has arisen between those brokers that
track and end-user citizens who are being tracked and manipulated, and whose individual
...
anonymisation and privacy tools. This is followed by an overview of
...
histories\cite{montjoye2013Unique}.


Obfuscating data (location or otherwise) can be done [only] if the data is 
known to be collected in the first place. Some knowledge about how the 
data collection is done -- what data is captured, when -- also helps improve 
the obfuscation. 
\todo{PDS can manage this by being a gatekeeper that only sends data requested 
by a service - would mean the pds knows what is requested at all times}

For some types of data and services might not allow the user to choose how 
much (if any) is captured and disclosed, nor do they allow the generation or 
modification of the data. Some examples include all official data 
collection [???] like census data, police reports, tax returns, health records, 
but also private organizations with a semi-official / regulated status 
\todo{is this even a word?} like banks and utility providers. The ``official'' 
status of this types of personal data means that altering it in any way, for 
ofuscation purposes, might be considered a criminal offence. For example 
obfuscating one's income source by adding chaff can be considered tax evasion, 
theft, money laundering, etc; obfuscating energy consumption data can lead to 
suspicion of growing marijuana or cooking meth (in some places), or running 
illegal servers (in other places). \todo{do we want to go further?} Even 
weirder, obfuscating family membership :) false number of children / parents / 
supported or supporting family members. 
